...
template <typename IteratorType> void RangeFilter<IteratorType>::advance()
{
#define TARG_FN "RangeFilter<IteratorType>::advance()"
    ...
    if (iv_current != iv_end)
    {
        ...
        {
            if ((!iv_pPredicate) || ((*iv_pPredicate)(*iv_current)))
            {
                break;
            }
        }
...
{
    if ((iv_current != iv_end) &&
        ((iv_pPredicate) && (!((*iv_pPredicate)(*iv_current)))))
    {
        advance();
    }
}
...
{
    return (iv_current != iv_end) ? &RangeFilter::notComparable : NULL;
}
